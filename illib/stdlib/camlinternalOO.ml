...
(* $Id: camlinternalOO.ml,v 1.5 2004/05/20 16:38:45 montela Exp $ *)
...
(*  if (Obj.magic obj_0 : bool) then obj_0 else begin *)
  if (Obj.size (Obj.repr obj_0))>0 then obj_0 else begin 
(* *)
    (* XXX Appel de [obj_block] *)
    let obj = Obj.new_block Obj.object_tag table.size in
    (* XXX Appel de [modify] *)
...
(*   if (Obj.magic obj_0 : bool) then obj else begin *)
  if (Obj.size (Obj.repr obj_0))>0 then obj else begin 
(* *)
    let inits = table.initializers in
