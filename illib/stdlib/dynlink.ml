...
(* $Id: dynlink.ml,v 1.6 2006/04/22 04:25:23 montela Exp $ *)

(* Dynamic loading of .cmxa/dll files *)
...
let startup_dll_without_dependencies file_name =
  ignore(
    camil_exec_toplevel_phrase_noarg file_name "MLTop" "startupAll"
  )

let loadfile file_name =
  (* ne traite que les cmxa pour l'instant *)
  if Filename.check_suffix file_name "cmxa" then
    let dll_name = (Filename.chop_extension (file_name))^".dll" in
      startup_dll_without_dependencies dll_name
  else raise(Error(Not_a_bytecode_file file_name))
